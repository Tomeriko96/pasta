...
- **Removed archive**: the `lg_menu/`, `lg_menu_v2/`, `lg_menu_v3/` signage boards and the `build_signage_usb.py`, `build_signage_video.js`, and `gen_signage_*.py` scripts were deleted (A/B-test experiments and the USB signage route). The live signage is now only `menu_screen.html` (deployed via Play via URL, see `SIGNAGE_PLAN.md`), and the attract-loop videos are built with `scripts/build_attract_video.js`.
- **GitHub Pages (public site)**: serves only a curated subset of the repo via a GitHub Actions workflow (`.github/workflows/pages.yml`): `index.html`, `favicon.svg`, `attract.html`, `menu_screen.html`, `images/`, `pdf-menus-weasyprint/`, `CNAME`, `.nojekyll`. The repo root is no longer the served site, so source files (`scripts/`, the source `attract_*.html` pages, `AGENTS.md`, `SIGNAGE_PLAN.md`, `README.md`, `signage_images/`, the print-menu HTML sources) are not reachable via `lacarbonara.nl`. Render the MP4/PDF locally and commit them; the workflow only copies what is already committed.
...
- **Attract video loop seam**: a naive render shows a ~1s blank at the loop point because the clip's first second is still loading fonts/images. `scripts/build_attract_video.js` fixes this by discarding a 2s lead-in (`LEAD_IN`) and crossfading the tail into a fully-rendered frame. If a residual black still appears between loops on the TV, it is the player firmware's own transition: set playback **Conversion Effect -> Off**.
...
- `ffmpeg`: renders the looping MP4s (H.264 + yuv420p) from the headless Chromium capture. `scripts/build_attract_video.js` resolves ffmpeg automatically: tries system PATH first, then falls back to the `imageio-ffmpeg` Python package via `uv run --with imageio-ffmpeg`. Install the fallback with `uv run --with imageio-ffmpeg true` (one-time, cached). No `sudo` needed.
- `node` + `playwright-core` (installed locally via `npm i`): drives headless Chromium for the video capture
- Chromium binary: cached at `/home/dev/.cache/ms-playwright/chromium-1228/chrome-linux64/chrome` (set via `EXEC` in `scripts/build_attract_video.js`). Install with `node node_modules/playwright-core/cli.js install chromium` if missing.
...
node scripts/build_attract_video.js                 # Looping attract MP4 (defaults to attract_v12_white_loop.html)
node scripts/build_attract_video.js attract_authentic.html attract-authentic-loop.mp4                 # Authentic attract (no QR)
node scripts/build_attract_video.js attract_authentic_QR.html attract-authentic-QR-loop.mp4           # Authentic attract with scan slide
```
